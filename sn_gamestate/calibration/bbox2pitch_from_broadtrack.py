...
class Bbox2Pitch(ImageLevelModule):
    ...
    def preprocess(self, image, detections: pd.DataFrame, metadata: pd.Series) -> Any:
        ...
        image_id_str =metadata["id"]  # '000001'
        image_id_int = int(image_id_str)  # 1
        video_id = metadata["video_id"]  # 'SNGS-116'

        # 2. Construct the file path to the JSON file
        path_dir = '/home/ziegler/Code_Misc/BroadTrack/outputs/test/'
        file_path = os.path.join(path_dir, f"SNGS-{video_id}.json")

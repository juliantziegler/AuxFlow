...
class GetInitialFrames(VideoLevelModule):
    ...
    def process(self, detections: pd.DataFrame, metadatas: pd.DataFrame):
        ...
        #score = metadatas['score'].to_numpy()
        ...
        list_dumb_init = [75, 225, 375, 525, 675]
        for dumb in list_dumb_init:
            initialize[dumb] = 1
        ...
        for region in regions:
            if len(region) <= 100:
                ...
                best_idx = max(region, key=lambda i: ious[i])
                ...
                while i < len(region):
                    ...
                    # pick best in window
                    best_in_window = max(window, key=lambda idx: ious[idx])
        ...
        for i in range(len(all_boundaries) - 1):
            ...
            if end - start >= 300:
                ...
                while j < end:
                    ...
                    best_idx = max(window, key=lambda idx: ious[idx])
                    initialize[best_idx] = 1

...
def track_homographies(
    ...
    for i in frame_indices:
        ...
        while True:
            ...
            succeeded = cv2.imwrite(f'/data2/debug/random/debug_img{i}.png', blended)
        ...
        if not got_nbjw_homm:
            if valid_img_points is not None:
                ...
                cv2.imwrite(f'/data2/debug/random/after_tracking_{i}.png', debug_img)
        if keypoints_this_frame is not None and not got_nbjw_homm:
            for key in keypoints_this_frame.keys():
                if key in key_dict.keys():
                    if keypoints_this_frame[key]['p'] >= 0.7:
            ...
            aux_image = np.repeat(aux_image, 1, axis=0)
            aux_pitch = np.repeat(aux_pitch, 1, axis=0)
